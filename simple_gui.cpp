...
struct AppState {
    ...
    void renderModel() {
        ...
        for (const Triangle& tri : model->triangles) {
            ...
            auto drawLine = [&](int x0, int y0, int x1, int y1, float z) {
                int dx = abs(x1 - x0);
                int dy = abs(y1 - y0);
                int sx = (x0 < x1) ? 1 : -1;
                int sy = (y0 < y1) ? 1 : -1;
                ...
                while (true) {
                    if (x0 >= 0 && x0 < MODEL_VIEW_WIDTH && y0 >= 0 && y0 < MODEL_VIEW_HEIGHT) {
                        if (z < zBuffer[y0][x0]) {
                            frameBuffer[y0][x0] = '#';
                            colorBuffer[y0][x0] = currentColor;
                            zBuffer[y0][x0] = z;
                        }
                    }
            ...
            drawLine(x1, y1, x2, y2, avgZ);
            drawLine(x2, y2, x3, y3, avgZ);
            drawLine(x3, y3, x1, y1, avgZ);
        }
